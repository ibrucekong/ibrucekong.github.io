...
var DayAddon = function(lunar, boy, girl, date, option) {
  // 璁剧疆鎭嬬埍寮€濮嬬殑鏃堕棿,鍙互绮剧‘鍒扮
  var love_date = new Date();
  love_date.setFullYear(date.y ? date.y : 2010, date.M ? date.M - 1 : 0, date.d ? date.d : 1); //鏃堕棿骞存湀鏃�
  love_date.setHours(date.h ? date.h : 0); //灏忔椂
  love_date.setMinutes(date.m ? date.m : 0); //鍒嗛挓
  love_date.setSeconds(date.s ? date.s : 0); //绉掑墠涓€浣�
  love_date.setMilliseconds(0); //绉掔浜屼綅
...
DayAddon.prototype.solarSpicial = function(y, m, d) {
  ...
  console.log(d == this.today.getDate())
...
DayAddon.prototype.display = function() {
  ...
  var title = '绁�' + this.lover + this.festival + '蹇箰锛�';
